...
int main(int argc, char *argv[])
{
    ...
    rosHandle.param("lc_pointset_save_path", pointset_csv_path, std::string("data/lidar_camera/point_set.csv"));
    rosHandle.param("error_analysis_save_path", error_anaylysis_csv_path, std::string("data/border_error_anaylysis.csv"));
    rosHandle.param("lc_extrinsics_save_path", extrinsics_path, std::string("result/lidar_camera/extrinsics.yaml"));
    ...
    while(ros::ok())
    {
        ...
        {   
            ...
            pointset_csv_operator.writePointsToCSVAppend(pc_center_rcv, img_center_rcv);

            std::vector<geometry_msgs::Point32> pc_centers_raw;
            std::vector<geometry_msgs::Point32> img_centers_raw;

            pointset_csv_operator.readPointsFromCSV(pc_centers_raw, img_centers_raw);
            ...
            for(auto& pc_center: pc_center_rcv)
            {
                Eigen::Vector3f corner_trans(pc_center.x, pc_center.y, pc_center.z);
                corner_trans = R * corner_trans + t;
                pc_center.x = corner_trans.x();
                pc_center.y = corner_trans.y();
                pc_center.z = corner_trans.z();
                corners_cal.polygon.points.emplace_back(pc_center);
            }
        ...
        if(command_received == "undo" || command_received == "delete_once" || key == 8)
        {
            ...
            std::vector<geometry_msgs::Point32> img_centers_rcv = rcv_img_corners_packet.points.polygon.points;
            std::vector<geometry_msgs::Point32> pc_centers_rcv = rcv_pc_corners_packet.points.polygon.points;
            ...
            geometry_msgs::PolygonStamped corners_cal;
            for(auto pc_center: pc_corners_rcv)
            {
                Eigen::Vector3f corner_trans(pc_center.x, pc_center.y, pc_center.z);
                corner_trans = R * corner_trans + t;
                pc_center.x = corner_trans.x();
