...
int main(int argc, char *argv[])
{
	...
	rosHandle.param("radar4d_process_pc_sub_topic", topic_rd_sub, std::string("/rslidar_points"));
    ...
    rosHandle.param("rc_pointset_save_path", pointset_csv_path, std::string("data/lidar_camera/point_set.csv"));
    rosHandle.param("error_analysis_save_path", error_anaylysis_csv_path, std::string("data/border_error_anaylysis.csv"));
    rosHandle.param("rc_extrinsics_save_path", extrinsics_path, std::string("result/lidar_camera/extrinsics.yaml"));
    ...
    while(ros::ok())
    {
        ...
        if(command_received == "capture" || key == 13)
        {
            ...
            pointset_csv_operator.writePointsToCSVAppend(rd_center_rcv, img_center_rcv);

            std::vector<geometry_msgs::Point32> rd_centers_raw;
            std::vector<geometry_msgs::Point32> img_centers_raw;

            pointset_csv_operator.readPointsFromCSV(rd_centers_raw, img_centers_raw);

            if(rd_centers_raw.size() < 3 || img_centers_raw.size() < 3)
            {
            ...
            R = CalTool::findRotationByICP(rd_center_refer, img_center_refer);
            t = CalTool::findTranslation(center_rd, center_img, R);

            extrinsics_yaml_opr.writeExtrinsicsToYaml(R, t);

            geometry_msgs::PolygonStamped corners_cal;
            for(auto& pc_corner: rd_center_rcv)
            {
